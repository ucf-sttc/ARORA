...
public class CarAI : MonoBehaviour
{
    ...
    public bool castRays = false;
    float rayAlpha = 0; // 0.5f
    ...
    void Start()
    {
        ...
        initialTopSpeed = maxTopSpeed;
        carController.Topspeed = initialTopSpeed;

        //Create navmesh agent children
        GameObject aiGo = this.transform.Find("NavmeshAgent").gameObject;
        agent = aiGo.GetComponent<NavMeshAgent>();
    ...
    float GetCarSpeed(){
        ...
        if (!castRays) return initialTopSpeed;
    ...
    void CastRay(Vector3 anchor, float angle, Vector3 dir, float length, out CarAI outCarAI, out float outHitDistance){
        ...
        //Draw raycast
        Debug.DrawRay(anchor, Quaternion.Euler(0, angle, 0) * dir * length, new Color(1, 0, 0, rayAlpha));
