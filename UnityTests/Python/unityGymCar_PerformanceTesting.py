...
def main(args : argparse.Namespace):
    ...
    shortest_path_side_channel = ShortestPathSideChannel()
    ...
    unity_env = UnityEnvironment(
        ...
        side_channels =[engine_side_channel, environment_side_channel, position_scan_side_channel, map_side_channel, float_properties_side_channel, navigable_side_channel, set_agent_position_side_channel, shortest_path_side_channel],
        #log_folder = "D:/Work",
    ...
    #message_output = unity_env.process_immediate_message(map_side_channel.build_immediate_request("binaryMap", [327, 266, 0.5]))
    ...
    unity_env.process_immediate_message(shortest_path_side_channel.build_immediate_request())
    ...
    for t in range(30001):
        ...
        if (t >= 0):
            action = [ 0.5, 1, -1] # throttle, steering, braking
        if (t > 300):
            action = [0.5, -1, -1]
        if (t > 301):
            action = [0, 0, 0]
        ...
        print(f"{t}> Pos: {observation[len(observation)-1][0:3]} Goal: {observation[len(observation)-1][10:13]} R: {reward}")
